...
async fn thread_metadata_update_rejects_empty_git_info_patch() -> Result<()> {
    ...
    assert_eq!(
        ...
        "metadata or gitInfo must include at least one field"
    );
...
#[tokio::test]
async fn thread_metadata_update_replaces_metadata_and_returns_updated_thread() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let codex_home = TempDir::new()?;
    create_config_toml(codex_home.path(), &server.uri())?;
    ...
    let metadata = BTreeMap::from([
        ("clientTag".to_string(), "updated".to_string()),
    ...
    let updated_thread_json = update_result
        ...
        .expect("thread/metadata/update result.thread must be an object");
    assert_eq!(
        updated_thread_json.get("metadata"),
    ...
    assert_eq!(read.metadata, updated.metadata);
